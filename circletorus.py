...
for k in ks:
    ...
    loss = DistTopLoss(torch.cdist(x, x), (0, 1), (1, 1), k, 2)
    ...
    plt.savefig("circletorus/circle_" + "k" + str(k) + ".eps")
...
for k in ks:
    ...
    for i in range(ys.shape[0]):
        ...
        ax.scatter(ys[i, :, 0], ys[i, :, 1], ys[i, :, 2], s=8, c=np.arange(x.shape[0]))
        ax.tick_params(bottom=False, labelbottom=False, left=False, labelleft=False)
    plt.savefig("circletorus/torus_" + "k" + str(k) + ".eps")
